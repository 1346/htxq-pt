...
function getCode(){					//获取手机验证码
	if ( !$("#login_daojishi").hasClass('mmm') ) {
		var phoneNumber = $("#userNumber").val();
				$.ajax({ url: window.location.protocol + "//" + window.location.host+"/peanut/captcha/sms/web/login?&cc=86&mcc=460&m=" + phoneNumber + "&platform=1",
// url: "http://test.api.htxq.net/peanut/captcha/sms/web/login?&cc=86&mcc=460&m=" + phoneNumber + "&platform=1",
...
function loginSuccess(thisO){			//登录
	...
	if( code.length == 6 ){
		...
		$.ajax({
		    type: 'POST',
		    async: true, 
		     url: window.location.protocol + "//" + window.location.host+"/peanut/customer/login?mobile=" + mobile + "&platform=1",
